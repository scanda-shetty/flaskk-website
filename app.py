...
common = {'first_name': 'Peter', 'last_name': 'Parker', 'alias': 'spiderman'}
...
def projects():
  ...
  if tag is not None:
    data = [
      project for project in data if tag.lower() in
      [project_tag.lower() for project_tag in project['tags']]
    ]
...
def project(title):
  ...
  if 'description' not in selected:
    ...
    selected['description'] = io.open(get_static_file(
      'static/%s/%s/%s.html' % (path, selected['link'], selected['link'])),
                                      "r",
                                      encoding="utf-8").read()
